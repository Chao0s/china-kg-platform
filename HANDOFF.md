...
## Latest / 最新
- Workspace housekeeping session (2026-07-22, no Hualong content changes): ① The reported umbrella-repo divergence (22 ahead / 2 behind origin) was a false alarm — `origin/main` (herman925) and `chao0s/main` are the FLAT `git subtree split --prefix="Hualong Platform"` export (2-commit orphan history), not umbrella mirrors; local `main`'s stale upstream tracking made git compare unrelated histories. Resolved by `git branch --unset-upstream main`; never merge those remotes into the umbrella. Currency check: the subtree-split hash of local main must equal both remote tips (verified equal, `cd0e487`). ② Umbrella `.gitignore` now also ignores `.gstack/`, `.planning/`, `.playwright-mcp/` tool scratch (commit `781da27`). ③ Gate's `dashboard-smoke` e2e was failing because the understand-anything plugin cache had moved to 2.9.3 with an unbuilt `packages/core`; fixed by `pnpm install` + `pnpm --filter @understand-anything/core build` in the plugin cache dir — the launcher does the same, this was the manual equivalent. Gate green after.
