...
- **[DECIDED] Backend = Alibaba Cloud** — RDS (relational), OSS (object storage for all files), VOD/MPS (managed video transcoding + image processing), REST API on FC/ECS behind a 备案'd HTTPS domain, CDN. → [ADR-0004](adr/0004-backend-cloudbase-vs-alibaba.md).
- **[DECIDED] Files live in object storage, never in the database** — the DB holds metadata + URLs only.
- **[DECIDED] Content moderation = WeChat `security.*` (mandatory)**, optionally plus 阿里云内容安全 as a second layer. → [ADR-0005](adr/0005-mandatory-content-moderation.md).
...
- **[DECIDED] Uploads designed for weak China networks** — resumable/chunked (OSS multipart), retries, progress UI, optional Wi-Fi-only, and a pending → uploaded → moderated → published status so a dropped connection never loses work.
...
- **[RISK] Compliance lead time is external and not fully controllable** — 备案 (1–20 working days) + WeChat 审核, and it cannot start until the 主体 is confirmed. Mitigation: start the compliance track on day 1; keep a **体验版 (trial) pilot** as the Sep 1 fallback for whitelisted users if public 审核 slips. → [ADR-0008](adr/0008-launch-timeline-and-pilot.md).
...
- **[OPEN] Legal subject (主体)** — kindergarten 事业单位法人 vs 教育局/镇政府. Unconfirmed; gates 备案/认证/类目. The PRD documents both paths; confirm before the compliance track starts. → [ADR-0010](adr/0010-legal-subject.md).
...
- **[OPEN] 长期订阅消息 eligibility** — confirm or design around one-time subscribe messages.
...
- **Per-feature grilling at plan-time** — for the gnarly seams (audit edge cases, 成长册 export, subscribe-message flows) right before building each.
- **五大领域 evaluation 量表 content** — the rubric items + scoring behind the 五维雷达图; content from the 教研 team, not a structural decision.

## I. Personalization & habit analytics / 个性化与习惯分析
> Captured 2026-06-21 via grill-with-docs. Adults-only (teachers / parents) habit signals → `兴趣画像` → `个性化推荐`. Spec: [PERSONALIZATION.md](PERSONALIZATION.md). Decision record: [ADR-0011](adr/0011-personalization-and-habit-analytics.md).
...
- **[DECIDED] Output surfaces** — teacher `为你推荐` shelf + re-ranked `资源库` / `案例库`; plus a de-identified aggregate insight tile on the MEASUREMENT admin dashboard.
- **[DECIDED] LLM hosting** — domestic model (通义千问 on Alibaba, [ADR-0004](adr/0004-backend-cloudbase-vs-alibaba.md)) under a data-processing agreement; no personal information to an overseas API.
- **[DECIDED] Retention** — raw signals on a rolling 24-month window; profile recomputed; immediate purge on withdrawal / account exit.
- **[INVARIANT] No `child_id` on personalization signals** — personalization events carry content attributes only, a stream separate from the MEASUREMENT.md metric events; outputs are preference-level and never infer about a child. This is the load-bearing control.
...
1. Legal 主体 (kindergarten 法人 vs 教育局/镇政府) — gates compliance; confirm first.
2. Minors' data retention period + consent wording.
3. Account onboarding flow (roster-invite vs self-register-approve).
4. Departed-teacher content ownership.
5. Education 类目 + 办学许可证 requirement.
6. 长期订阅消息 eligibility.
7. PC backend operators (who).
8. Success-metric targets.
9. Media cost ceiling.
10. `算法备案` — does teacher-only per-user `个性化推荐` (internal staff tool) require the filing?
11. Domestic-LLM data-processing agreement (DPA) terms for the `兴趣画像` summarization.
12. Parent profiling — whether and how (v2 decision).
13. 24-month personalization-data retention — purpose justification acceptable?
