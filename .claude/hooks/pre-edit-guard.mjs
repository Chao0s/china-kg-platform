...
//   ask   -> ASK THE USER to confirm    (permissionDecision: "ask",  exit 0)   <-- the requested behavior
...
const DEFAULT_GUARD = {
  deny: ['.git/', 'LICENSE', '*.lock'],
  ask: ['docs/glossary.json', 'harness/structure/app-structure.json', 'harness/harness.config.json', '.githooks/', '.github/workflows/', '.claude/settings.json'],
  warn: ['harness/', 'package.json', '.claude/hooks/'],
};
...
async function main() {
  ...
  if (hit(guard.ask)) {
    decision('ask', 'This is a governance / source-of-truth file (guard.ask) — editing it changes what the harness enforces. Confirm you intend this change. Run `npm run gate` afterward.');
  }
  if (hit(guard.warn)) {
    warn('NOTE: editing harness/CI/config infrastructure (guard.warn) — changes here affect the gate. Review carefully and run `npm run gate`.');
  }
  process.exit(0);
}
