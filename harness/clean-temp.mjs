...
// can only remove genuinely-ignored scratch. Used by the `temp-janitor` subagent, which
// always lists and asks the user to confirm before calling this with --apply.
//
...
const configured = (cfg.tempCleanup && cfg.tempCleanup.paths) || ['tests/.tmp', 'harness/.report'];
...
for (const i of deletable) console.log(`  ${i.type === 'dir' ? '📁' : '·'} ${i.path}  ${c.dim(`(${i.files} file(s), ${human(i.bytes)})`)}`.replace('📁', 'dir '));
for (const i of refused) console.log(c.yellow(`  ! ${i.path} is git-tracked — refusing to delete`));
if (!apply) {
  console.log('');
  console.log(c.dim('Dry run. Re-run with --apply to delete, or use the temp-janitor subagent (it confirms first).'));
  process.exit(0);
}
