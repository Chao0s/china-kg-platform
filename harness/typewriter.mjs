#!/usr/bin/env node
// typewriter.mjs — House-style content tool for the China KG Platform. Two modes:
//
//   GENERATE:  node harness/typewriter.mjs new <type> "<title>" [--out PATH]
//              types: adr | prd-section | design-section | research | readme-section
...
//                - bilingual docs (.zh-CN or mixed) should keep EN/简中 heading counts comparable (P2)
//
...
- **Module / 模块:** <home | resource-library | ...>
...
- **Positive / 正面:** <...>
- **Negative / 负面:** <...>
- **Compliance / 合规:** <PIPL / 内容安全 / 备案 implications, if any>
`,
  'prd-section': (t, d) => `### ${t}
...
(Describe in evocative, natural language — translate technical values into physical description, always with the exact token in parentheses.)
`,
...
function lint() {
  ...
  const files = [];
  for (const t of (targets.length ? targets : ['docs', 'README.md', 'README.zh-CN.md', 'CONTEXT.md', 'CLAUDE.md', 'AGENTS.md'])) {
  ...
  for (const file of [...new Set(files)]) {
    ...
    for (let i = 0; i < rawLines.length; i++) {
      ...
      // English sentence with an embedded 中文 term (e.g. "Case library / 案例库, alias 课程库")
