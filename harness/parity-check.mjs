...
for (const pair of pairs) {
  ...
  if (!enExists || !zhExists) {
    if (enExists || zhExists) {
      findings.push({ severity: 'P1', title: `Bilingual pair incomplete: ${pair.en} / ${pair.zh} — one side is missing`, fix: 'create the missing twin so both languages exist' });
    }
    continue;
  }
...
console.log('');
console.log(blocking ? c.red(`✗ ${blocking} parity finding(s).`) : c.yellow(`${findings.length} finding(s).`));
